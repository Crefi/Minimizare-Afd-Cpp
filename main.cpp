...
//functia asta este folosita o singrua data pentru a verifica daca o stare apartine multimii de stari finale sau nu
...
int main()
{
	...
	for (int i = 0; i < n; i++) // initializez matricea
		for(int j = 0; j < n; j++)
			a[i][j] = '-';

	for(int x = 0; x < nr_fin; x++) //marchez in matrice linia starilor finale
	{
	...
	while(gasit == true) //marchez in matrice pana cand nu imi mai modifica nimic
	{
		gasit = false;
		for(int i = 0; i < n-1; i++)
		{
	...
	cout << "\nMatrice:" << endl;
	for (int i = 0; i < n; i++) // afisare matrice minimizata
	{
		for(int j = 0; j < i; j++)
			cout << a[i][j] << " ";
		cout << endl;
	...
	for (int i = 0; i < n; i++) // unde exista - in matrice  notez in h ca sunt eliminate
	{
		for(int j = 0; j < i; j++)
			if(a[i][j] == '-')
				h[i] = 0;
	...
	cout << endl << "Starile eliminate: " << endl << endl;
	for(int i = 0; i < n; i++)
		if(h[i] == 0)
			cout  << i << endl;
	cout << endl;
